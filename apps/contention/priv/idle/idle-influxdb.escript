...
main(_) ->
	io:format("usage: statscheck.escript NODE COOKIE FILENAME\n"),
	erlang:halt(1).
...
-define(test_func, spinsleep_dirty).
...
init(Parent, Filename) ->
	State = #state{
		...
		tests = #{
			<<"1x1ms">> => {'Elixir.Contention', ?test_func, [1000 * 1, erlang:system_info(schedulers) * 1]},
			<<"10x1ms">> => {'Elixir.Contention', ?test_func, [1000 * 1, erlang:system_info(schedulers) * 10]},
			<<"100x1ms">> => {'Elixir.Contention', ?test_func, [1000 * 1, erlang:system_info(schedulers) * 100]},
			<<"1000x1ms">> => {'Elixir.Contention', ?test_func, [1000 * 1, erlang:system_info(schedulers) * 1000]},
			<<"10000x1ms">> => {'Elixir.Contention', ?test_func, [1000 * 1, erlang:system_info(schedulers) * 10000]}
		}
	},
	ok = connect(State#state.node),
	loop(State).

loop(State=#state{parent=Parent, node=Node, test_ref=TestRef}) ->
	receive
...
handle(State0=#state{prev_ts=T0, prev_da=D0}, T1, D1) ->
	Events0 = [],
	T0Sec = ((T0 + 500000000) div 1000000000),
	T1Sec = ((T1 + 500000000) div 1000000000),
	{State1, Events1} =
		case T1Sec - T0Sec of
			D when D > 2 ->
...
make_errors(State0, [I | Is], Timestamp, Events) ->
	Event0 = {Timestamp + (I * 1000000000), #{ <<"error">> => 1 }},
...
make_points(State0, [I | Is], Timestamp, Data, Events) ->
	{State1, Event} = make_point(State0, Timestamp + (I * 1000000000), Data),
...
notify(State, Event) ->
	Points = points(State, Event),
	Command = io_lib:format("curl -i -XPOST 'http://127.0.0.1:8086/write?db=mydb' --data-binary '~s'", [
		Points
	]),
...
create(_State, _Timestamp) ->
	true.

points(State, {Timestamp, Event0}) ->
	Event = lists:sort(maps:to_list(Event0)),
	points(State, Timestamp, Event, []).

points(_State, _Timestamp, [], []) ->
	<<>>;
points(_State, _Timestamp, [], [_ | Acc]) ->
	erlang:iolist_to_binary(lists:reverse(Acc));
points(State=#state{filename=Filename}, Timestamp, [{Key, Value} | Event], Acc0) ->
	Point = io_lib:format("~s,test=~s value=~w ~w", [
		Key,
		Filename,
		Value,
		Timestamp
	...
	Acc1 = [$\n, Point | Acc0],
	points(State, Timestamp, Event, Acc1).
...
unixtime({MegaSecs, Secs, MicroSecs}) ->
	(MegaSecs * 1000000000000000) + (Secs * 1000000000) + (MicroSecs * 1000).
