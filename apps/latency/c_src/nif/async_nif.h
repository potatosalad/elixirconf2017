...
static struct async_nif_req_entry *async_nif_reuse_req(struct async_nif_state *async_nif);
static void async_nif_recycle_req(struct async_nif_req_entry *req, struct async_nif_state *async_nif);
...
#define ASYNC_NIF_INIT(name) static ErlNifMutex *name##_async_nif_coord = NULL;

#define ASYNC_NIF_LOAD(name, env, priv)                                                                                            \
    do {                                                                                                                           \
...
#define ASYNC_NIF_UNLOAD(name, env, priv)                                                                                          \
    do {                                                                                                                           \
        if (!name##_async_nif_coord)                                                                                               \
...
#define ASYNC_NIF_UPGRADE(name, env)                                                                                               \
    do {                                                                                                                           \
        if (!name##_async_nif_coord)                                                                                               \
 ...
 */
static struct async_nif_req_entry *
...
async_nif_enqueue_req(struct async_nif_state *async_nif, struct async_nif_req_entry *req, int hint)
{
    ...
    ERL_NIF_TERM reply =
        enif_make_tuple2(req->env, ATOM_OK, enif_make_tuple2(req->env, ATOM_ENQUEUED, enif_make_double(req->env, pct_full)));
...
async_nif_load(ErlNifEnv *env)
{
    ...
    async_nif = malloc(sizeof(struct async_nif_state) + sizeof(struct async_nif_work_queue) * num_queues);
    if (!async_nif)
        return NULL;
    memset(async_nif, 0, sizeof(struct async_nif_state) + sizeof(struct async_nif_work_queue) * num_queues);
